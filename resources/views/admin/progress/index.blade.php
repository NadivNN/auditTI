...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold leading-tight text-gray-700 dark:text-sky-300 tracking-wide animate-fadeInDown">
            Admin - Progress Pengisian Kuesioner
        </h2>
    </x-slot>
    ...
    <div class="py-12 dark:bg-slate-950">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div
                class="p-6 sm:p-8 bg-white dark:bg-slate-800/70 dark:backdrop-blur-md shadow-xl dark:shadow-blue-950/40 sm:rounded-xl dark:border dark:border-blue-700/30 animate-fadeInUp">
                <h3 class="mb-8 text-2xl font-bold tracking-tight text-gray-800 dark:text-sky-300">
                    Daftar Progress User
                </h3>

                @if (session('success'))
                    <div class="mb-6 p-4 text-sm rounded-lg text-green-700 bg-green-100 dark:bg-green-700/20 dark:text-green-300 border border-green-600/30 animate-fadeInDown"
                        role="alert">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                <div class="overflow-x-auto shadow-lg rounded-xl dark:border dark:border-slate-700/80">
                    <table class="min-w-full">
                        <thead class="dark:bg-slate-700/80">
                            <tr>
                                <th scope="col"
                                    class="px-6 py-4 text-xs font-semibold tracking-wider text-left text-gray-500 dark:text-sky-300 uppercase border-b-2 dark:border-slate-600">
                                    Pengguna</th>
                                <th scope="col"
                                    class="px-6 py-4 text-xs font-semibold tracking-wider text-left text-gray-500 dark:text-sky-300 uppercase border-b-2 dark:border-slate-600">
                                    Progress</th>
                        ...
                        <tbody class="bg-white dark:bg-slate-800">
                            @forelse ($users as $user)
                                <tr class="dark:hover:bg-slate-700/60 transition-colors duration-150">
                                    <td
                                        class="px-6 py-4 text-sm text-gray-800 dark:text-sky-100 whitespace-nowrap border-b dark:border-slate-700">
                                    ...
                                    <td
                                        class="px-6 py-4 text-sm text-gray-700 dark:text-gray-200 whitespace-nowrap border-b dark:border-slate-700">
                                    ...
                                    <td class="px-6 py-4 space-x-4 text-sm font-medium whitespace-nowrap">
                                        <a href="{{ route('admin.progress.show', $user) }}"
                                            class="font-semibold text-sky-500 hover:text-sky-400 transition-colors duration-150"
                                        ...
                                        <a href="{{ route('admin.progress.downloadPDF', $user) }}"
                                            class="font-semibold text-emerald-600 hover:text-emerald-500 dark:text-emerald-400 dark:hover:text-emerald-300 transition-colors duration-150"
