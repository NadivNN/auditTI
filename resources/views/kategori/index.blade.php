    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg shadow">
                <form method="GET" action="{{ route('kategori.index') }}"> {{-- Ganti 'kategori.index' dengan nama route Anda --}}
                    <div class="flex flex-col sm:flex-row items-end space-y-2 sm:space-y-0 sm:space-x-3">
                        <div>
                            <label for="cobit_item_id_filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Filter Berdasarkan Cobit Item:</label>
                            <select name="cobit_item_id" id="cobit_item_id_filter" class="mt-1 block w-full sm:w-auto pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md dark:bg-gray-900 dark:border-gray-600 dark:text-gray-100">
                                ...
                                @if(isset($cobitItems) && $cobitItems->count() > 0)
                                    @foreach ($cobitItems as $cobitItemOption)
                                        <option value="{{ $cobitItemOption->id }}" {{ (string)$selectedCobitItemId === (string)$cobitItemOption->id ? 'selected' : '' }}>
                                            {{ $cobitItemOption->nama_item }} {{-- Ganti 'nama_item' jika nama field berbeda --}}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <a href="{{ route('kategori.index') }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:text-gray-300 dark:bg-gray-700 dark:border-gray-500 dark:hover:bg-gray-600 dark:focus:ring-offset-gray-800">
                            Reset Filter
                        </a>
                        @endif
                    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-300">Nama</th>
                            <th class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-300">Cobit Item</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                        {{-- Pastikan variabel $kategoris ada dan tidak kosong sebelum loop --}}
                            ...
                            @foreach ($kategoris as $kategori)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                                        {{ $kategori->cobitItem->nama_item ?? 'N/A' }} {{-- Menampilkan nama Cobit Item --}}
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
                                        <a href="{{ route('kategori.edit', $kategori->id) }}" class="mr-4 text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-600">
                                            Edit
                                        </a>
