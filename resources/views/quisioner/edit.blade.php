        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="p-6 overflow-hidden bg-white shadow sm:rounded-lg dark:bg-gray-800">
                <form action="{{ route('quisioner.update', $quisioner->id) }}" method="POST">
                    ...
                    <div class="mb-6">
                        <label for="pertanyaan" class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Pertanyaan:</label>
                        <textarea name="pertanyaan" id="pertanyaan" rows="4" required
                            class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100">{{ old('pertanyaan', $quisioner->pertanyaan) }}</textarea>
                        @error('pertanyaan')
                    ...
                    <div class="mb-6">
                        <label for="level_id" class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Level:</label>
                        <select name="level_id" id="level_id" required
                            ...
                            @foreach ($levels as $level)
                                <option value="{{ $level->id }}" {{ old('level_id', $quisioner->level_id) == $level->id ? 'selected' : '' }}>
