    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg shadow">
                <form method="GET" action="{{ route('quisioner.index') }}"> {{-- Ganti 'quisioner.index' dengan nama route Anda --}}
                    <div class="flex flex-col sm:flex-row items-end space-y-2 sm:space-y-0 sm:space-x-3">
                        <div>
                            <label for="level_id_filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Filter Berdasarkan Level:</label>
                            <select name="level_id" id="level_id_filter" class="mt-1 block w-full sm:w-auto pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md dark:bg-gray-900 dark:border-gray-600 dark:text-gray-100">
                                <option value="">Semua Level</option>
                                @if(isset($levelsForFilter) && $levelsForFilter->count() > 0)
                                    @foreach ($levelsForFilter as $levelOption)
                                        <option value="{{ $levelOption->id }}" {{ (string)$selectedLevelId === (string)$levelOption->id ? 'selected' : '' }}>
                                            {{ $levelOption->nama_level }} ({{ $levelOption->kategori->nama ?? 'N/A' }})
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        @if($selectedLevelId)
                        <a href="{{ route('quisioner.index') }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:text-gray-300 dark:bg-gray-700 dark:border-gray-500 dark:hover:bg-gray-600 dark:focus:ring-offset-gray-800">
                            Reset Filter
                        </a>
                        @endif
                    </div>
            ...
            <div class="overflow-hidden bg-white shadow sm:rounded-lg dark:bg-gray-800">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-300">Pertanyaan</th>
                            <th class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-300">Level - Kategori</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                        @if(isset($quisioners) && $quisioners->count() > 0)
                            @foreach ($quisioners as $quisioner)
                                <tr>
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap dark:text-gray-100">
                                        {{ $quisioner->id }}
                                    </td>
                                    <td class="w-1/2 px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                                        {{ $quisioner->pertanyaan }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap dark:text-gray-300">
                                        {{ $quisioner->level->nama_level ?? 'N/A' }} - {{ $quisioner->level->kategori->nama ?? 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
                                        <a href="{{ route('quisioner.edit', $quisioner->id) }}" class="mr-4 text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-600">
                                            Edit
                                        </a>
                                        <form action="{{ route('quisioner.destroy', $quisioner->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @else
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    Tidak ada data quisioner ditemukan.
                                    @if($selectedLevelId)
                                        Coba reset filter.
                                    @endif
                                </td>
                ...
                {{-- Paginasi Links --}}
                 @if(isset($quisioners) && $quisioners->hasPages())
