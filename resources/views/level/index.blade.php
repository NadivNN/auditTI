    ...
    <div class="py-12 dark:bg-slate-950">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-8">
                ...
                <form method="GET" action="{{ route('level.index') }}">
                    <div class="flex flex-col sm:flex-row items-end gap-4">
                        <div class="w-full sm:w-auto sm:flex-grow">
                            <label for="kategori_id_filter" class="block mb-1 text-sm font-semibold text-gray-700 dark:text-sky-200">Filter Berdasarkan Kategori:</label>
                            <select name="kategori_id" id="kategori_id_filter" class="custom-select-dark mt-1 block w-full pl-4 pr-10 py-2.5 text-base border-gray-300 dark:border-slate-600 dark:bg-slate-700/80 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 sm:text-sm rounded-lg shadow-sm">
                                <option value="">Semua Kategori</option>
                                @foreach ($kategoris as $kategoriOption) {{-- Pastikan variabel $kategoris dikirim dari controller --}}
                                    <option value="{{ $kategoriOption->id }}" {{ (string)$selectedKategoriId === (string)$kategoriOption->id ? 'selected' : '' }}>
                                        {{ $kategoriOption->nama }} ({{ $kategoriOption->cobitItem->nama_item ?? 'N/A'}})
                                    </option>
                                @endforeach
                            </select>
                        </div>
            ...
            <div class="p-0 bg-white dark:bg-slate-800/70 dark:backdrop-blur-md shadow-xl dark:shadow-blue-950/40 sm:rounded-xl dark:border dark:border-blue-700/30 animate-fadeInUp animation-delay-100" style="opacity:0;">
                    ...
                    <table class="min-w-full">
                        <thead class="dark:bg-slate-700/80">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-xs font-semibold tracking-wider text-left text-gray-500 dark:text-sky-300 uppercase border-b-2 dark:border-slate-600">Nama Level</th>
                                <th class="px-6 py-4 text-xs font-semibold tracking-wider text-left text-gray-500 dark:text-sky-300 uppercase border-b-2 dark:border-slate-600">Kategori (Cobit Item)</th>
                            ...
                            @forelse ($levels as $level)
                                <tr class="dark:hover:bg-slate-700/60 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap border-b dark:border-slate-700">
                                        ...
                                        <span class="block text-xs text-gray-500 dark:text-slate-400">({{ $level->kategori->cobitItem->nama_item ?? 'N/A' }})</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium whitespace-nowrap border-b dark:border-slate-700 space-x-4">
                                        <a href="{{ route('level.edit', $level->id) }}" class="font-semibold text-sky-600 hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-200 transition-colors duration-150">
                                            Edit
                                        </a>
                                        <form action="{{ route('level.destroy', $level->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-slate-400">
                                        ...
                                        @if($selectedKategoriId)
                                            Coba reset filter atau pilih kategori yang lain.
                                        @else
                                            Anda bisa menambahkan level baru.
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
                ...
                {{-- Paginasi Links --}}
                @if($levels->hasPages())
                    <div class="px-0 py-3 mt-0 bg-transparent dark:bg-transparent sm:px-0 rounded-b-xl">
